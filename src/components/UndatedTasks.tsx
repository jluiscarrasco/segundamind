...
export function UndatedTasks({ tasks, projects, areas, onEditEntity, onSetTaskDate }: UndatedTasksProps) {
  ...
  return (
    <div className="bg-card rounded-xl border border-border shadow-card overflow-hidden flex flex-col">
      <div className="px-5 py-4 border-b border-border flex items-center gap-2">
        ...
        <h2 className="text-sm font-semibold text-foreground">Tareas sin fecha</h2>
          ...
          items.map((item, i) => {
            ...
            >
              ...
              <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                <button
                  onClick={(e) => { e.stopPropagation(); onSetTaskDate(item.task.id, todayKey); }}
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); onSetTaskDate(item.task.id, tomorrowKey); }}
                  className="text-[11px] font-medium text-muted-foreground hover:text-primary hover:bg-primary/10 rounded px-1.5 py-0.5 transition-colors"
                >
                  Mañana
                </button>
                <button
                  onClick={(e) => { e.stopPropagation(); onSetTaskDate(item.task.id, weekKey); }}
                  className="text-[11px] font-medium text-muted-foreground hover:text-primary hover:bg-primary/10 rounded px-1.5 py-0.5 transition-colors"
                >
                  +7d
                </button>
